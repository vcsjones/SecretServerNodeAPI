...
Client.prototype.authenticateHosted = function(username, password, organizationCode) {
    ...
    return new Promise(function(fulfill, reject) {
        self._soap.Authenticate(args, function(err, result) {
            ...
            else if (result.AuthenticateResult.Errors) {
                var ssErrors = normalizeError(result.AuthenticateResult.Errors);
                if (ssErrors.length > 0) {
                    reject(ssErrors);
                } else {
                    fulfill(result.AuthenticateResult.Token);
                }
            }
            else fulfill(result.AuthenticateResult.Token);
        });
...
Client.prototype.searchSecrets = function() {
    ...
    return new Promise(function(fulfill, reject) {
        self._soap.SearchSecrets(args, function(err, result) {
            if (err) reject(err);
            else if (result.SearchSecretsResult.Errors) {
                var err = normalizeError(result.SearchSecretsResult.Errors);
                if (err.length > 0) {
                    reject(err);
                }
...
Client.prototype.getSecretTemplates = function(token) {
    ...
    return new Promise(function(fulfill, reject) {
        self._soap.GetSecretTemplates(args, function(err, result) {
            if (err) reject(err);
            else if (result.GetSecretTemplatesResult.Errors) {
                var err = normalizeError(result.GetSecretTemplatesResult.Errors);
                if (err.length > 0) {
                    reject(err);
                }
